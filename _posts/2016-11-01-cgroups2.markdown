...
The preceeding was an intentionally deceitful title, by now the least you should
have learned is that this is not a blog about joga, when I say *stress* I mean
...
* Line 4 sets the limit of memory this control group can reserve on 256MB
...
* In line 14 the extra instruction added to the trap command deletes the new
  control group so that the clean up is complete
...
Just like the [previous post]({{ site.url }}/cgroups-1) this was a well
intentioned attempt to serve as an introduction to the concepts behind control
groups, hope it helps somebody other than my future self and as you can imagine
to distill such a complex subject in a few posts requires extreme
